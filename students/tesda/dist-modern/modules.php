...
<body data-page="dashboard" class="admin-layout">
    ...
    <div class="row g-4">
        <?php
        $userId = $_SESSION['userId']; // Make sure session has userId

        // Fetch active exams
        $exams = $conn->query("SELECT * FROM exams WHERE status = 'Active' ORDER BY chapterId ASC");
        ...
        if ($exams && $exams->num_rows > 0):
            while ($exam = $exams->fetch_assoc()):
                ...
                $taken = $conn->query("
                    ...
                    FROM exam_results 
                    WHERE exam_id = {$exam['id']} AND userId = $userId
                    LIMIT 1
                ")->fetch_assoc();
        ?>
            <div class="col-12 col-md-6 col-lg-4">
                <div class="card shadow-sm border-0 h-100">
                    <div class="card-body">
                        <h6 class="text-secondary mb-1">Chapter <?= htmlspecialchars($exam['chapterId']) ?></h6>
                        <h5 class="card-title text-primary fw-bold"><?= htmlspecialchars($exam['title']) ?></h5>
                        <p class="card-text text-muted mb-3"><?= htmlspecialchars($exam['description']) ?></p>

                        <?php if ($taken): ?>
                            <p class="text-success mb-2">
                                Score: <?= $taken['score'] ?> / <?= (int)$exam['totalItems'] ?> 
                                (<?= round($taken['percentage'], 2) ?>%)
                            </p>
                        ...
                        <?php else: ?>
                            <div class="d-flex justify-content-between align-items-center">
                                <small class="text-muted">Items: <?= (int)$exam['totalItems'] ?></small>
                                <a href="takeExam.php?examId=<?= $exam['id'] ?>" class="btn btn-sm btn-outline-success">
